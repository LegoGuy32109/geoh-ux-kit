...
export const Home = () => (
  <>
    <div className='card' style={{ marginBottom: 24, maxWidth: 680 }}>
      <h2 style={{ margin: '0 0 12px', fontSize: 'var(--web-text-size-extra-large)', color: 'var(--web-bold-color)' }}>
        Mocked screens
      </h2>
      {SCREENS.length === 0 ? (
        ...
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          {SCREENS.map((screen) => (
            <li key={screen.path} style={{ marginBottom: 6 }}>
              ...
              <span style={{ color: 'var(--web-text-color-light)' }}> — {screen.path}</span>
            </li>
          ))}
        </ul>
      )}
    ...
    <div className='notice'>
      ...
      <p style={{ marginTop: 0 }}>
        Run <code>npm run new:screen /my-feature</code>, then point <code>meta.nav</code> at a group and an icon:
      </p>
      <pre>
        {`export const meta = {
