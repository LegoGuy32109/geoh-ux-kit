 ...
 * Scaffold a screen.
 *
 *   npm run new:screen /broadcast-studio                      new route, top-level nav row
 *   npm run new:screen /broadcast-studio SuperAdmin GrAnnounce  nested under a group, with an icon
 *   npm run new:screen /clients                               path already in nav.json — no nav block needed
 *
...
if (!path?.startsWith('/')) {
  console.error('Usage: npm run new:screen /some/path [ParentGroupKey] [IconName]')
  process.exit(1)
}
...
const flat = groups.flatMap((group) => [group, ...(group.items ?? [])])
...
const knownIcons = new Set([...iconSource.matchAll(/^\s{2}([A-Z][A-Za-z0-9]+),?$/gm)].map((m) => m[1]))
if (icon !== undefined && !knownIcons.has(icon)) {
  ...
  console.error(`  ${[...knownIcons].sort().join(', ')}`)
  process.exit(1)
}

const title = navEntry?.label ?? path.split('/').filter(Boolean).pop().replace(/(^|-)(\w)/g, (_, d, c) => (d ? ' ' : '') + c.toUpperCase())
...
if (navEntry !== undefined) {
  ...
  console.log('Pass a group key second to nest it, e.g. npm run new:screen ' + path + ' SuperAdmin GrAnnounce')
}
